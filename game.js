...
function makeP(x,face,cpu,char){
 ...
 return {x,y:G,w:92,h:140,vx:0,vy:0,face,cpu,char,name:c.jp,hp:c.hp,maxHp:c.hp,meter:25,
  ...
  combo:0,comboT:0,guardStun:0,airSpecial:false,super:"",superT:0,superHit:false,aiT:0,aiMode:""};
}
...
function control(p,e){
 if(p.down)return;if(p.guardStun){p.guardStun--;p.vx=0;return;}if(p.stun){p.stun--;return;}
 const ch=roster[p.char]; if(p.comboT>0)p.comboT--;else if(!p.atk)p.combo=0;
 p.guard=getIn(p,"guard")&&grounded(p)&&!p.atk&&!p.super;
 let sp=(p.guard?1.25:4.35)*ch.spd;p.vx=0;
 if(getIn(p,"left")){p.vx=-sp;if(!p.atk&&!p.super)p.face=-1;}
 if(getIn(p,"right")){p.vx=sp;if(!p.atk&&!p.super)p.face=1;}
 if((getIn(p,"jumpUp")||getIn(p,"jumpBack")||getIn(p,"jumpForward"))&&grounded(p)&&!p.guard){
   p.vy=-16.8;const side=8.2*ch.spd;
   if(getIn(p,"jumpBack")){p.vx=-p.face*side;} else if(getIn(p,"jumpForward")){p.vx=p.face*side;} else if(getIn(p,"left"))p.vx=-side; else if(getIn(p,"right"))p.vx=side; else p.vx=0;
 }
 ...
 if(getPressed(p,"special")&&!p.atk&&!p.spcd&&!p.guard){let dir=e.x>=p.x?1:-1;p.face=dir;p.atk=grounded(p)?30:34;p.type="special";p.active=true;p.spcd=grounded(p)?86:104;addMeter(p,3);if(grounded(p))bullets.push({o:p,x:p.x+dir*76,y:p.y-92,w:50,h:30,vx:dir*8.8,vy:0,life:100,big:1,ref:0});else{p.airSpecial=true;p.vx=dir*8.5;p.vy=Math.max(p.vy,6.5);bullets.push({o:p,x:p.x+dir*72,y:p.y-82,w:48,h:28,vx:dir*6.2,vy:5.2,life:70,big:1,air:1,ref:0});msg="AIR SPECIAL!";msgT=20;}}
}

function physics(p){
 if(p.cd>0)p.cd--;if(p.spcd>0)p.spcd--;if(p.throwcd>0)p.throwcd--;if(p.inv>0)p.inv--;
 if(updateSuper(p)){p.x=clamp(p.x,55,W-55);return;}
 if(p.down){p.downT--;p.vy+=.72;p.x+=p.vx;p.y+=p.vy;p.vx*=.965;if((p.x<65&&p.vx<0)||(p.x>W-65&&p.vx>0)){p.x=clamp(p.x,65,W-65);p.vx*=-.28;p.vy=-5;shake=18;screenFlash=8;msg="WALL CRASH!";msgT=24;spark(p.x,p.y-75,46,"#fff");}if(p.y>G){p.y=G;p.vy=0;p.vx*=.86;}if(p.downT<=0){p.down=false;p.stun=0;p.type="";p.vx=0;p.inv=25;}p.x=clamp(p.x,55,W-55);return;}
 if(p.atk>0)p.atk--;else{p.type="";p.active=false;p.airSpecial=false;}
 p.vy+=.72;if(!grounded(p)&&!p.airSpecial)p.vx*=.985;if(p.airSpecial)p.vx*=.995;p.x+=p.vx;p.y+=p.vy;if(p.y>G){p.y=G;p.vy=0;p.airSpecial=false;}p.x=clamp(p.x,55,W-55);
}
